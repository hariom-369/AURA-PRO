...
Authentication is **email + password and Google Sign-In** — passwords are hashed with Argon2id and never stored or returned in plain text; Google identities are verified server-side (`google-auth-library`) and never auto-merged into an existing password account without an explicit, authenticated "link" step. `/login`/`/register` are protected by Cloudflare Turnstile bot-check (optional — only enforced when configured) alongside the existing per-route rate limiting. Register offers a Buyer/Seller intent picker (pure UI framing, never trusted for authorization — every account is created as `role: 'customer'` regardless). See **[docs/AUTHENTICATION.md](docs/AUTHENTICATION.md)** for the full account model, the session/"remember me" design, and the account-linking policy. Also: cart, wishlist, saved addresses, product reviews with verified-purchase detection, coupons, full order lifecycle (PENDING → CONFIRMED → SHIPPED → DELIVERED, plus RETURN_REQUESTED/RETURNED/REFUNDED), Stripe Checkout with webhook-confirmed payment, admin product CRUD with Cloudinary image upload, admin order/return management, and an analytics dashboard (revenue trend, top products, low-stock alerts) built with `recharts`.
...
│   ├── emailService.js         Resend SDK wrapper for order + password-reset emails (no-ops cleanly if RESEND_API_KEY unset)
│   ├── stripeClient.js         Shared Stripe client (null if unconfigured)
│   ├── googleAuthService.js    Verifies Google ID tokens server-side (google-auth-library)
│   ├── turnstileService.js     Verifies Cloudflare Turnstile challenge tokens server-side
│   ├── passwordResetService.js Single-use, hashed password-reset tokens
...
node seeds/seedAdmin.js <email>                           # promote an existing account (matched by email) to admin
```

`seedData.js` **wipes** Products/Users/Wishlists/Addresses in the target database — only run it against a dev database. It seeds no user accounts: register once through the app to create an account, then run `seedAdmin.js` with that same email to promote it.
...
| `RESEND_API_KEY` | Order/status emails and password-reset emails via [Resend](https://resend.com) — see [docs/EMAIL.md](docs/EMAIL.md) for setup, including the required sending-domain verification | Order emails are logged instead of sent; `forgot-password` still responds successfully (its response never reveals account existence) but no reset email actually arrives |
| `GOOGLE_CLIENT_ID` (backend) + `VITE_GOOGLE_CLIENT_ID` (frontend) | Google Sign-In — see [docs/AUTHENTICATION.md](docs/AUTHENTICATION.md) for Google Cloud Console setup | The "Continue with Google" button doesn't render; email/password still works fully |
| `TURNSTILE_SECRET_KEY` (backend) + `VITE_TURNSTILE_SITE_KEY` (frontend) | Cloudflare Turnstile bot protection on register/login/forgot-password — see [docs/AUTHENTICATION.md](docs/AUTHENTICATION.md) | Turnstile enforcement is skipped entirely (fine for local dev; production should set this) |
...
Full list with defaults: [`server/.env.example`](server/.env.example) and [`client/.env.example`](client/.env.example). Order-email/password-reset-email setup and testing: [docs/EMAIL.md](docs/EMAIL.md). Marketplace/seller setup: [docs/MARKETPLACE.md](docs/MARKETPLACE.md). Full auth architecture, Google/Turnstile setup: [docs/AUTHENTICATION.md](docs/AUTHENTICATION.md).
...
Backend tests cover: server-authoritative pricing (GST, shipping thresholds, coupon caps), the full order-creation flow (stock reservation/rollback, auth gating), the Stripe webhook handler (mocked Stripe client — no live API calls), auth middleware, the full authentication surface (register/login/forgot-reset-password/change-password/Google sign-in/account-linking/Turnstile — see [docs/AUTHENTICATION.md](docs/AUTHENTICATION.md#automated-tests) for the full breakdown), the AI graceful-degradation contract, and the full marketplace surface — seller onboarding/admin-review lifecycle (including illegal-status-transition rejection), payout-field encryption, cross-seller data isolation on products/orders, commission-ledger math, and ledger reversal on refund. They run with `GEMINI_API_KEY`/`STRIPE_SECRET_KEY`/`RESEND_API_KEY`/Cloudinary/`GOOGLE_CLIENT_ID`/`TURNSTILE_SECRET_KEY` all intentionally unset, to exercise the fallback paths by default; `server/tests/aiClient.test.js`, `server/tests/emailDelivery.test.js`, `server/tests/googleAuth.test.js`, and `server/tests/turnstile.test.js` separately mock the Gemini, Resend, Google, and Cloudflare SDKs/APIs to test the real request/response handling without live credentials.

Client tests cover the full `/login`, `/register`, `/forgot-password`, and `/reset-password` flows (validation, loading/disabled states, remember-me storage location, error handling), and the Google Sign-In / Turnstile widget components against mocked `window.google`/`window.turnstile` — see [docs/AUTHENTICATION.md](docs/AUTHENTICATION.md#automated-tests) for the full breakdown.
...
- Passwords hashed with Argon2id (`@node-rs/argon2`), `select: false` in the schema so a plain query never even fetches the hash
- Google ID tokens verified server-side (signature + audience) via the official `google-auth-library` — a client-asserted email/name/id is never trusted
- Cloudflare Turnstile bot-check on register/login/forgot-password, verified server-side against Cloudflare's API with a backend-only secret key
- Password-reset tokens: cryptographically random, only their SHA-256 hash is stored, 30-minute expiry, single-use
- Login and forgot-password responses are enumeration-safe — never reveal whether an email is registered
